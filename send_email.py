...
try:
    ...
    yagmail = None  # allow DRY_SEND runs without yagmail installed
...
def main():
    ...
    # Optional: render-only mode to produce artifacts without sending email.
    if os.environ.get("DRY_SEND", "0").lower() in ("1", "true", "yes"):
        from pathlib import Path as _Path
        from email.message import EmailMessage as _EmailMessage
        from email.utils import formatdate as _formatdate
        _Path("artifacts").mkdir(exist_ok=True)
        # Save the exact HTML body (Outlook consumes this)
        (_Path("artifacts") / "email_out.html").write_text(body_html, encoding="windows-1252")
        # Construct an .eml with text + HTML alternative for easy local open
        _msg = _EmailMessage()
        _msg["From"] = EMAIL_USER
        _msg["To"] = EMAIL_TO
        _msg["Subject"] = f"{DEFAULT_TITLE} — {datetime.now().strftime('%d %b %Y')}"
        _msg["Date"] = _formatdate(localtime=True)
        _msg.set_content("This message contains an HTML part.")
        _msg.add_alternative(body_html, subtype="html")
        (_Path("artifacts") / "email_out.eml").write_bytes(_msg.as_bytes())
        print("DRY_SEND=1: wrote artifacts/email_out.html and artifacts/email_out.eml")
        return

    to_list = [addr.strip() for addr in re.split(r"[,\s]+", EMAIL_TO) if addr.strip()]

    if yagmail is None:
        raise SystemExit('yagmail not installed; install requirements or set DRY_SEND=1 for render-only')
